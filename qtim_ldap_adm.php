...
$tt=0;
$oVIP->selfurl = 'qtim_ldap_adm.php';
$oVIP->selfname = 'LDAP/AD stettings';
$oVIP->exiturl = 'qti_adm_secu.php';
...
if ( isset($_POST['ok']) && $tt==0 )
{
  ...
  // Enable module (m_ldap=0 means disable, m_ldap=qtim_ldap_login.php means enabled
...
if ( isset($_POST['ok']) && $tt>0 )
{
  ...
  if ( $tt==2 )
  {
    ...
    {
      ++$step;
      $b = qtim_ldap_bind($username,$password);
      if ( $b )
      {
...
<tr>
<th><label for="newuser">Status</label></th>
';
...
<table class="t-data horiz">
<tr>
<th><label for="newuser">Login users</label></th>
<td colspan="2">
...
';
echo '<tr>
<th><label for="newuser">',L('Information'),'</label></th>
<td colspan="2">
...
On first login, a local profile is created for the user having a valid ldap id.<br/>
...
<p class="submit"><input type="hidden" name="tt" value="',$tt,'"/><input type="submit" name="ok" value="',L('Save'),'"/></p>
</form>
';
}
...
<table class="t-data horiz">
<tr>
<th><label for="m_ldap_host">Host</label></th>
...
<tr>
<th><label for="m_ldap_rdn">Login DN</label></th>
...
<table class="t-data horiz">
<tr>
<th style="vertical-align:top"><label for="m_ldap_bind">When searching</label></td>
...
<tr>
<th><label for="m_ldap_rdn">Search RDN</label></th>
...
<tr>
<th><label for="m_ldap_s_filter">Search filter</label></th>
...
<tr>
<th><label for="m_ldap_s_info">Requested info</label></th>
...
if ( $tt==2 )
{
  if ( function_exists('ldap_connect') ) { echo '<p class="submit"><input type="hidden" name="tt" value="',$tt,'"/><input type="submit" name="ok" value="',$L['test'],'"/></p>'; } else { echo '<p>Test not possible</p>'; }
}
else
{
  echo '<p class="submit"><input type="hidden" name="tt" value="',$tt,'"/><input type="submit" name="ok" value="',L('Save'),'"/></p>';
}
