...
{
  $i=0;
  foreach(sMem::Get('sys_sections') as $id=>$name) if ( IsMapSection($id) ) ++$i;
  return $i;
}

// INITIALISE

$oVIP->selfurl = 'qtim_map_adm.php';
$oVIP->selfname = 'Map';
...
foreach(array('m_map_gkey','m_map_gcenter','m_map_gzoom','m_map_gfind','m_map_gbuttons') as $strValue)
{
  ...
  {
  $arr = GetSettings('param="'.$strValue.'"',true);
...
if ( isset($_POST['ok']) )
{
  ...
  else
  {
    ...
    $_SESSION[QT]['m_map_gbuttons'] = substr($_POST['maptype'],0,1).(isset($_POST['streetview']) ? '1' : '0').(isset($_POST['map']) ? '1' : '0').(isset($_POST['scale']) ? '1' : '0').(isset($_POST['fullscreen']) ? '1' : '0').(isset($_POST['mousewheel']) ? '1' : '0');
    ...
    if ( $_SESSION[QT]['m_map_gsymbol']!=='0' && file_exists('qtim_map/'.$_SESSION[QT]['m_map_gsymbol'].'_shadow.png') ) $_SESSION[QT]['m_map_gsymbol'] .= ' '.$_SESSION[QT]['m_map_gsymbol'].'_shadow';
...
if ( file_exists('qtim_map/config.php') ) require 'qtim_map/config.php';
...
<tr>
<th style="width:120px"><label for="m_map_gkey">Google API key</label></td>
<td class="t-data" colspan="2"><input type="text" id="m_map_gkey" name="m_map_gkey" size="70" maxlength="100" value="',$_SESSION[QT]['m_map_gkey'],'" onchange="bEdited=true;"/></td>
</tr>
';
...
echo '<tr>
<th style="width:120px">',$L['map_API_ctrl'],'</td>
...
&nbsp;<input type="checkbox" id="fullscreen" name="fullscreen"'.(substr($_SESSION[QT]['m_map_gbuttons'],4,1)=='1' ? QCHE : '').' style="vertical-align: middle" onchange="bEdited=true;"/><label for="fullscreen">',$L['map_Ctrl_fullscreen'],'</label>
...
<tr>
<th style="width:120px">',$L['map_default_symbol'],'</td>
<td style="width:70px;text-align:center"><img id="markerpicked" title="default" alt="i" src="',($strSymbol==='0' ? 'bin/css/gmap_marker.png' : 'qtim_map/'.$strSymbol.'.png' ),'"/></td>
<td>
...
{
echo '<input type="radio" name="m_map_gsymbol" value="'.$strFile.'" id="symbol_'.$strFile.'"'.($strSymbol===$strFile ? QCHE : '').' onchange="radioHighlight(this.id);bEdited=true;"/><label for="symbol_'.$strFile.'"><img id="image_'.$strFile.'" alt="i" class="marker'.($strSymbol==$strFile ? ' checked' : '').'" title="'.$strName.'" src="qtim_map/'.$strFile.'.png"/></label>'.PHP_EOL;
}
echo '</div>
</td>
</tr>
<tr>
<th style="width:120px;">',L('Sections'),'</td>
<td colspan="2"><strong>
',CountMapSections(),'</strong>/',count(sMem::Get('sys_sections')),
(IsMapSection('U') ? ' '.L('and').' '.L('Users') : ''),
(IsMapSection('S') ? ' '.L('and').' '.L('Search_result') : ''),
' &middot; <a href="',Href('qtim_map_adm_sections.php'),'">',L('map_define_sections'),'...</a>
</td>
</tr>
</table>
';
...
<table class="t-data horiz">
<tr>
<th style="width:120px;"><label for="m_map_gcenter">',$L['map_Center'],'</label></td>
...
<tr>
<th style="width:120px;"><label for="m_map_gzoom">',$L['map_Zoom'],'</label></td>
<td class="t-data">
...
<tr>
<th style="width:120px;">',$L['map_Background'],'</td>
...
<tr>
<th style="width:120px;"><label for="m_map_gfind">',$L['map_Address_sample'],'</label></td>
<td class="t-data">
...
{
  $oCanvas = new cCanvas();
  $oCanvas->Header(L('map_canmove').' | <a class="gmap" href="javascript:void(0)" onclick="undoChanges(); return false;" />'.L('map_undo').'</a>' );
