 ...
 * ------------
 * Class cMapPoint
 * QTgcanmap QTgmapHasKey QTgmapApi QTgempty QTgemptycoord
 * QTgmapMarker QTgmapMarkerMapTypeId QTgmapMarkerIcon
...
function getMapSectionsSettings($jMapSections='')
{
  ...
  // When $json is empty, the function uses the session variable $_SESSION[QT]['m_map'].
  ...
  // TIPS - To force a reload from the config file, just use unset($_SESSION[QT]['m_map']) then getMapSectionsSettings()
  ...
  if ( json_last_error()!==JSON_ERROR_NONE) { echo '<p class="error">Unable to read map configurations from '.APP.'m_map/config_map.php</p>'; return false; }
  return $o;
}
...
class cCanvas
{
  ...
  public function cCanvas($id='map_canvas',$class='')  { $this->canvas = '<div'.cCanvas::idclass($id,$class).'></div>'.PHP_EOL; }

  public function Render($handler=false,$id='',$class='gmap')
	{
	  if ( isset($_SESSION[QT]['m_map_hidelist']) && $_SESSION[QT]['m_map_hidelist'] ) 
    {
    ...
    $str = '<div'.cCanvas::idclass($id,$class).'>'.PHP_EOL;
		...
		if ( $handler )
		{
			...
			{
      $str .= '<div id="canvashandler" class="canvashandler"><a class="canvashandler" href="'.Href().'?'.GetURI('hidemap').'&amp;showmap"><i class="fa fa-caret-down"></i> '.L('map_Show_map').'</a></div>'.PHP_EOL;
			}
			else
			{
      $str .= '<div id="canvashandler" class="canvashandler"><a class="canvashandler" href="'.Href().'?'.GetURI('showmap').'&amp;hidemap"><i class="fa fa-caret-up"></i> '.L('map_Hide_map').'</a></div>'.PHP_EOL;
			}
		}
  ...
  public function Header($arrExtData=array(),$arrEditCommands=array(),$id='',$class='header')
	{
	  ...
	  {
          $this->header .= '<p'.cCanvas::idclass($id,$class).' style="margin:0 0 4px 0"><a class="gmap" href="javascript:void(0)" onclick="zoomToFullExtend(); return false;">'.L('map_zoomtoall').'</a> | '.L('Show').' <select class="gmap" id="zoomto" name="zoomto" size="1" onchange="gmapPan(this.value);">';
		      $this->header .= '<option value="'.$_SESSION[QT]['m_map_gcenter'].'"> </option>';
		      $i=0;
    ...
    // Commands
	  ...
	  if ( is_array($arrEditCommands) && count($arrEditCommands)>0 )
	  {
			...
			foreach($arrEditCommands as $str)
			{
				...
				if ( $str==='add' ) $str=' | <a href="javascript:void(0)" onclick="createMarker(); return false;" title="'.L('map_H_pntadd').'">'.L('map_pntadd').'</a>';
				if ( $str==='del' ) $str=' | <a href="javascript:void(0)" onclick="deleteMarker(); return false;">'.L('map_pntdelete').'</a>';
				// other values
				$this->header .= $str;
			}
			$this->header .= '</p>';
  ...
  public function Footer($str='find',$id='',$class='footer')
	{
     if ( $str==='find' )
     {
     $str = L('map_addrlatlng').' <input type="text" size="24" id="find" name="find" class="small" value="'.$_SESSION[QT]['m_map_gfind'].'" title="'.L('map_H_addrlatlng').'" onkeypress="qtKeypress(event,\'mapfindit\');"/><i id="mapfindit" class="fa fa-search fa-lg" title="'.L('Search').'" onclick="showLocation(document.getElementById(\'find\').value,null);"></i>';
     }
     if ( !empty($str) )
     {
...
function QTgmapHasKey()
{
  return !empty($_SESSION[QT]['m_map_gkey']); // Returns true when map google key exists
}
...
function QTgemptycoord($a)
{
  ...
  if ( is_array($a) )
  {
    if ( !isset($a['x']) || !isset($a['y'])) return true;    
    if ( QTempty($a['x']) ) return true;
    if ( QTempty($a['y']) ) return true;
    return false;
  }
...
function QTgmapMarkerIcon($gsymbol=false,$gshadow=false,$qte_gprinticon=true,$qte_gprintshadow=false)
{
  ...
  {
    case 'pushpin':
      $str = 'icon: new google.maps.MarkerImage("'.APP.'m_map/'.$gsymbol.'.png",new google.maps.Size(32,32),new google.maps.Point(0,0),new google.maps.Point(10,32)),';
      if ( $gshadow ) $str .= 'shadow: new google.maps.MarkerImage("'.APP.'m_map/'.$gshadow .'.png",new google.maps.Size(59,32),new google.maps.Point(0,0),new google.maps.Point(10,32)),';
      break;
    case 'point':
     $str = 'icon: new google.maps.MarkerImage("'.APP.'m_map/'.$gsymbol.'.png",new google.maps.Size(32,32),new google.maps.Point(0,0),new google.maps.Point(16,32)),';
      if ( $gshadow ) $str .= 'shadow: new google.maps.MarkerImage("'.APP.'m_map/'.$gshadow .'.png",new google.maps.Size(59,32),new google.maps.Point(0,0),new google.maps.Point(16,32)),';
     break;
    default:
     $str = 'icon: new google.maps.MarkerImage("'.APP.'m_map/'.$gsymbol.'.png",new google.maps.Size(32,32),new google.maps.Point(0,0),new google.maps.Point(16,16)),';
      if ( $gshadow ) $str .= 'shadow: new google.maps.MarkerImage("'.APP.'m_map/'.$gshadow .'.png",new google.maps.Size(59,32),new google.maps.Point(0,0),new google.maps.Point(16,16)),';
     break;
  }
  return $str;
}
...
function QTgmapMarkerMapTypeId($qti_gbuttons)
{
  switch($qti_gbuttons)
  {
  case 'S':
