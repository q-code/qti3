...
function qtim_ldap_bind($username,$password)
{
  $b = false;
  global $error;
...
function qtim_ldap_search($username)
{
  global $error;
  $error = '';
  $mail = '';
...
function qtim_ldap_profile($username,$password,$mail='')
{
  if ( empty($mail) ) $mail = qtim_ldap_search($username); // seach the user's mail from ldap (may be empty)
